...
// FIXED: Updated import paths to use Libraries/Inspector instead of src/private/inspector
let getInspectorDataForViewAtPoint: any;
let InspectorOverlay: any;
...
if (__DEV__) {
  try {
    ...
    console.warn("Inspector modules not available:", error);
...
export function BundleInspector(props: BundleInspectorProps) {
  ...
  const onTouchPoint = (locationX: number, locationY: number) => {
    if (!getInspectorDataForViewAtPoint) return;
  ...
  return (
    <View style={styles.container} ref={appContainerRootViewRef}>
      ...
      {inspectorEnabled && InspectorOverlay && (
